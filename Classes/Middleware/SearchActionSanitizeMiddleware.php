 ...
 */
class SearchActionSanitizeMiddleware implements MiddlewareInterface
{
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        $requestBody = (array)$request->getParsedBody();

        // Check if the necessary path with search exists in the request body
        if (ArrayUtility::isValidPath($requestBody, 'tx_yellowpages2_directory/search')) {
            // Retrieve and modify the search data
            $search = ArrayUtility::getValueByPath(
                $requestBody,
                'tx_yellowpages2_directory/search'
            );
